        ...
        <div className="overflow-hidden rounded-xl border border-gray-100 dark:border-gray-800">
            ...
            >
                ...
                <div className="flex items-center gap-2">
                    <span className="text-xs text-gray-400">{kec.kelurahan.length} kel</span>
                    <svg
            ...
            {open && (
                <div className="grid grid-cols-2 gap-1 bg-white p-3 dark:bg-gray-900/50">
                    {kec.kelurahan.map((kel) => (
                        <div key={kel.kode} className="rounded-lg px-3 py-1.5 text-xs text-gray-600 dark:text-gray-400">
                            {kel.nama}
                            {kel.kodePos && (
                                <span className="ml-1 text-gray-400 dark:text-gray-600">{kel.kodePos}</span>
                            )}
                        </div>
                    ))}
                </div>
...
export default function WilayahIndex({
    ...
    const provinsiNama = provinsiList.find((p) => p.kode === selectedProvinsi)?.nama ?? selectedProvinsi;
    ...
    return (
        <Layout>
                    ...
                    {searchResults.length > 0 ? (
                        <div className="space-y-1">
                            {searchResults.map((item) => (
                                <div key={item.kode} className="rounded-xl bg-gray-50 px-4 py-3 dark:bg-gray-800/50">
                                    <div className="flex items-center justify-between gap-2">
                                        <span className="font-medium text-gray-900 dark:text-gray-100">{item.nama}</span>
                                        <span className={['shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium capitalize',
                                            TIPE_COLOR[item.tipe] ?? 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'
                                        ].join(' ')}>{item.tipe}</span>
                                    </div>
                                    <p className="mt-0.5 text-xs text-gray-400">{item.kota} · {item.provinsi}</p>
                                </div>
                            ))}
                        </div>
                    ) : (
                        <p className="text-sm text-gray-400">Tidak ada hasil untuk "{searchQuery}"</p>
                    )}
                </div>
            )}
                ...
                <div className="mb-4 flex items-center gap-2 text-sm">
                    ...
                    {showKota ? (
                        <>
                            ...
                            <span className="text-gray-300 dark:text-gray-700">/</span>
                            <span className="font-medium text-gray-900 dark:text-gray-100">{kotaDetail?.nama}</span>
                        </>
                    ) : (
                        <span className="font-medium text-gray-900 dark:text-gray-100">{provinsiNama}</span>
                    )}
                </div>
            )}
            ...
            {showKota && (
                <div className={['grid gap-5', cuaca ? 'lg:grid-cols-[1fr_340px]' : ''].join(' ')}>
                    {/* Kecamatan list */}
                    <div className="rounded-2xl border border-gray-200/60 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                        <p className="mb-4 text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                            Kecamatan
                        ...
                        <div className="rounded-2xl border border-gray-200/60 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                            ...
                            <div className="space-y-2">
                                {cuaca.prakiraan.map((p, i) => (
                                    ...
                                    >
                                        <span className="text-2xl leading-none">{weatherEmoji(p.cuaca)}</span>
                                        <div className="min-w-0 flex-1">
                                            <div className="flex items-center justify-between">
                                                <span className="text-xs text-gray-400">{p.waktu}</span>
                                                <span className="font-bold text-gray-900 dark:text-gray-100">{p.suhu}</span>
                                            </div>
                                            <p className="mt-0.5 truncate text-xs text-gray-500 dark:text-gray-400">{p.cuaca}</p>
                                            <p className="text-xs text-gray-400">💧{p.kelembapan} · 💨{p.kecepatanAngin}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
